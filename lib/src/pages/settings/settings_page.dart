...
class _SettingPageState extends State<SettingPage> {
  ...
  Widget build(BuildContext context) {
    ...
    notifService = Provider.of<NotificationService>(context);
    ...
    return Scaffold(
                        ...
                        children: [
                          Expanded(
                              child: TextInfo(
                                  text: '${student.pp} %',
                                  title: 'Progessão',
                                  titleColor: MainTheme.white,
                                  backgroundColor: MainTheme.orange,
                                  textColor: MainTheme.white,
                                  onTap: () => showModalBottomSheetDefault(
                                      context,
                                      'Equivale ao percentual realizado no curso pela carga horária das disciplinas aprovadas.',
                                      title: 'Percentual de Progressão (PP)'))),
                          ...
                          Expanded(
                              child: TextInfo(
                                  ...
                                  onTap: () => showModalBottomSheetDefault(
                                      context,
                                      'Equivale a nota média do estudante no curso.',
                                      title: 'Percentual de Rendimento (PR)')))
                        ],
                      ),
                    ),
                    CopyCard(
                      text: student.ra,
                      ico: Icon(Icons.school, color: MainTheme.white),
                      color: MainTheme.white,
                      backgroundColor: MainTheme.lightBlue,
                      animationColor: MainTheme.white,
                    ),
                    NavigationButton(
                        ...
                        child: const StudentCardPage()),
                  ],
                ),
              ),
              Divider(color: Theme.of(context).colorScheme.onPrimary),
              NavigationButton(
                  text: 'Configuração de Exibição',
                  icon: Icons.contrast,
                  child: const ViewSettings()),
              NavigationButton(
                  text: 'Configuração de Segurança',
                  icon: Icons.security,
                  child: const SecuritySettingPage()),
              NavigationButton(
                    ...
                    switch (snapshot.connectionState) {
                      case ConnectionState.done:
                        return Align(
                          ...
                          child: Text(
                              '"Já tá no Siga?" v${snapshot.data!.version}',
      ...
      bottomNavigationBar: SizedBox(
        ...
        child: ElevatedButton(
          onPressed: () async {
            showModalBottomSheetConfirmAction(context, 'Deseja mesmo sair?',
                () async {
              ...
              try {
                ...
                notifService.cancelNotitifications();
                if (mounted) {
