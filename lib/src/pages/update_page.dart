...
class _UpdatePageState extends State<UpdatePage> {
  ...
  _initializeUpdate() async {
    setState(() => inUpdate = true);
    Iterable<OtaStatus> errorStatus =
        OtaStatus.values.where((e) => e.toString().contains('ERROR'));
    ...
    Map<OtaStatus, String> errorMsg = {
      ...
      OtaStatus.CHECKSUM_ERROR:
          ' ao verificar a integridade do arquivo baixado!',
      OtaStatus.PERMISSION_NOT_GRANTED_ERROR: ', sem permissões necessárias'
    };

    try {
      OtaUpdate()
          .execute(update.update.link,
        ...
        (OtaEvent event) {
          ...
          if (errorStatus.contains(currentEvent!.status)) {
            ...
            Fluttertoast.showToast(
                msg: 'Ocorreu um erro${errorMsg[currentEvent!.status]}');
          }
        },
      );
    } catch (e) {
  ...
  List<Widget> _generateChildren() {
    List<Widget> children = [
      Row(
        ...
        children: [
          Flexible(
              child: Text('Já tá no Siga? v${update.update.version}',
