                  ...
                  for(int j=0; j<int.parse(value.toString());j++){
                    await view.runJavaScriptReturningResult("document.getElementById('Grid1ContainerTbl').getElementsByTagName('tbody')[0].getElementsByTagName('tr')[$i].getElementsByTagName('span')[$j].textContent").then((value){
                      if(j==0){
                        ...
                        discipline['average'] = discipline['average'] =='--'?'--':discipline['average']!.replaceAll(" ", '').length==5?discipline['average']!.toString().substring(0,discipline['average']!.length-3):discipline['average']!.toString().replaceAll(" ", "").substring(0,discipline['average']!.length-2);
                      }else if(j==4){
